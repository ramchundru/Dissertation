...
lr_predicted_proba = lr_model.predict_proba(X_test)
# printing the model training time
print("--- %s seconds ---" % (time.time() - start_time))

# Accuracy Calculation
from sklearn.metrics import accuracy_score
accscore = accuracy_score(Y_test,lr_predicted_values)
print ("Accuracy is %s" % accuracy_score(Y_test,lr_predicted_values))

# Precision Calculation
from sklearn.metrics import precision_score
prscore = precision_score(Y_test,lr_predicted_values, average='weighted')
print("Precision Score is %s" % prscore)

# Recall Calculation
from sklearn.metrics import recall_score
rescore = recall_score(Y_test,lr_predicted_values, average='weighted')
print("Recall Score is %s" % rescore)

# F1-score calculation
from sklearn.metrics import f1_score
f1score = f1_score(Y_test,lr_predicted_values, average='weighted')
print("F-1 Score is %s" % f1score)

from sklearn.metrics import roc_curve,roc_auc_score,precision_recall_curve
#keeping probabilities for true prediction
fpr, tpr, thresholds = roc_curve(Y_test,lr_predicted_proba[:,1])
auc = roc_auc_score(Y_test,lr_predicted_values)
precision, recall, thresholds = precision_recall_curve(Y_test,lr_predicted_proba[:,1])
